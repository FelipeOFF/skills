...
It is a business-rule-scoped, non-technical, auto-triggering variant of [Matt Pocock's `grill-me`](https://github.com/mattpocock/skills/tree/main/skills/productivity/grill-me). It keeps the proven `grilling` core loop and changes four things: **when** it fires, **who** it's for, **how** it asks, and **which language** it asks in.
...
In Claude Code (and any other agent you install it on), the skill now watches for business-rule changes and grooms them before they land. You can also invoke it on demand by typing `/groom-me`.

## The difference from Matt Pocock's `grill-me`

`groom-me` is **not** a rename of `grill-me` — it shares its engine but is pointed at a different job.
...
# Pick specific agents non-interactively (Claude Code + Codex)
...
`groom-me` derives from the `grilling` / `grill-me` primitive by **[Matt Pocock](https://github.com/mattpocock/skills)**, used under the MIT License. This repo is MIT-licensed too — see the [root LICENSE](../../LICENSE). Hack on it, adapt it, make it yours.
